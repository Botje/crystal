...
(define (new-variables parms env)
  (append (map (lambda (x) (pair x (new-variable x))) parms) env))
...
(define *primitives*
  (list
    (pair 'car (lambda (args)
                 (and (= (length args) 1)
                      (pair? (car args))
                      (quot (car (car args))))))
    (pair 'cdr (lambda (args)
                 (and (= (length args) 1)
                      (pair? (car args))
                      (quot (cdr (car args))))))
    (pair '+ (lambda (args)
    ...
    (pair 'not (lambda (args)
                 (and (= (length args) 1)
                      (quot (not (car args))))))
    (pair 'null? (lambda (args)
                   (and (= (length args) 1)
                        (quot (null? (car args))))))
    (pair 'pair? (lambda (args)
                   (and (= (length args) 1)
                        (quot (pair? (car args))))))
    (pair 'symbol? (lambda (args)
                     (and (= (length args) 1)
